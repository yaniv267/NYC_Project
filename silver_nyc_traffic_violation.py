...
from nyc_schema import silver_parking_violations
...
# =========================
# Define retention limit (e.g., keep only the last 24 months)
...
df_cleaned = (
    ...
    # B. Extract hour from violation_time (e.g., "0409A")
    .withColumn("hour", regexp_extract(col("violation_time"), r"^(\d{2})", 1).cast("int"))
    # C. Clean street name - trim whitespace and uppercase
    .withColumn("street_name", trim(upper(col("street_name"))))  
    # D. Handle dates and timestamps
...
df_silver_final.write \
    ...
    .mode("append") \
    .save("s3a://spark/silver/nyc_parking_violation/")
