...
def process_street_search(message):
    ...
    try:
        ...
        for s_name, boro in street_combos:
            ...
            cur.execute(f"""
                SELECT violation_desc, tickets_count, risk_level, is_camera, lat, lon, issue_day_name, time_of_day
                FROM gold_traffic_violations 
                WHERE {match_cond} 
            ...
            # קואורדינטות התאונות נמצאות באינדקס 6 ו-7
            ...
            if c311:
                peak_311 = f"{int(c311[2]):02d}:00" if c311[2] is not None else "N/A"
            ...
            if violation:
                ...
                res += f"• ⚠️ *Peak Risk:* `{violation[6]}` during `{violation[7]}`\n"
            ...
            if crash:
                ...
                p_hour = crash[13]
                p_day = crash[14]
                time_desc = get_time_of_day_description(p_hour)
                res += f"💥 *CRASH HISTORY*\n"
                res += f"• Safety: `{crash[10]}`\n"
                res += f"• Total Crashes: `{crash[2]}`\n"
                res += f"• ⚠️ *Peak Timing:* `{p_day}` | `{time_desc}`\n"
                res += f"• Injured: `{crash[3]}` | Killed: `{crash[4]}`\n"
