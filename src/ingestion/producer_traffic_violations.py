...
# --- SAFETY LIMITS ---
DAYS_BACK = 30 # צומצם מ-90 ל-2 ימים כדי לרוץ מהר כ-Daily Batch
MAX_TOTAL_RECORDS = 20000 # מגבלת עצירת חירום כדי שה-DAG לא ייתקע לנצח
...
# 2. DATE RANGE
# ==========================================
start_date = (latest_api_date - timedelta(days=DAYS_BACK)).strftime("%Y-%m-%d")
end_date_str = latest_api_date.strftime("%Y-%m-%d")
...
def fetch_and_send_all():
    ...
    while True:
        # עצירת חירום כדי למנוע ריצה אינסופית ב-Airflow
        if total_sent >= MAX_TOTAL_RECORDS:
            print(f"🛑 Reached safety limit of {MAX_TOTAL_RECORDS} records. Stopping ingestion.")
            break
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"❌ API Error {response.status_code}: {response.text}")
                raise Exception(f"API Error {response.status_code}") # זריקת שגיאה כדי ש-Airflow יידע על הנפילה
            ...
            print(f"❌ Critical error during fetching: {str(e)}")
            # אנו זורקים את השגיאה החוצה כדי שה-BashOperator ייכשל ו-Airflow יבצע Retry לפי ההגדרות ב-DAG
            raise e
