...
# OPTIMIZATION: Combine into gold_base ONCE using Broadcast Joins
gold_base = tickets_prepared.join(F.broadcast(addresses_clean), ["street_key", "borough_code"], "left") \
                            .join(F.broadcast(codes_clean), ["violation_code_int"], "left")

# OPTIMIZATION: Cache the joined table to RAM to prevent re-reading from MinIO during counts
gold_base = gold_base.cache()

# Calculate Match Rate directly from the cached Gold Base (Saves a double Join!)
total_count = gold_base.count()
matched_count = gold_base.filter(F.col("lat").isNotNull()).count()
...
if final_count > 0:
    ...
    # OPTIMIZATION: partitionBy("borough") added for massive query speedup
    gold_final.write.mode("overwrite").partitionBy("borough").parquet("s3a://spark/gold/parking_violations_new/")
